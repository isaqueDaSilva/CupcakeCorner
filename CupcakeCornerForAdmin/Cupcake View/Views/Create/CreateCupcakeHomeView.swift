...
extension CreateCupcakeView {
    final class ViewModel: ObservableObject {
        ...
        let persistenceStorage: SwiftDataService<Cupcake>
        ...
        func create(_ completationHandler: @escaping () -> Void) {
            Task(priority: .background) {
                do {
                    ...
                    let cupcakeDecoded = try decoder.decode(Cupcake.Get.self, from: data)
                    
                    let cupcake = Cupcake(from: cupcakeDecoded)
                    
                    try persistenceStorage.create(new: cupcake)
        ...
        init(inMemoryOnly: Bool = false) {
            ...
            persistenceStorage = SwiftDataService<Cupcake>(inMemoryOnly: inMemoryOnly)
        }
    }
}
